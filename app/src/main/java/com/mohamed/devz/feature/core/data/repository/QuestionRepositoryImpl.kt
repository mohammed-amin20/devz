...
class QuestionRepositoryImpl @Inject constructor(
    ...
    override suspend fun getByAccountIds(
        ...
        return try {
            val questions = remoteDataSource.question.getQuestionsByAccountIds(accountIds, offset, limit)
            Result.Success(questions.map { it.toDomain() })
        } catch (e: PostgrestRestException) {
            Result.Error(Error.Unknown("Database error"))
        } catch (e: IOException) {
    ...
    override suspend fun getByTags(
        tags: List<String>,
        offset: Int,
        limit: Int,
    ): Result<List<Question>, Error> {
        return try {
            val questions = remoteDataSource.question.getQuestionsByTags(tags, offset, limit)
            Result.Success(questions.map { it.toDomain() })
        } catch (e: PostgrestRestException) {
            Result.Error(Error.Unknown("Database error"))
        } catch (e: IOException) {
    ...
    override suspend fun getPinnedQuestions(): Result<List<Question>, Error> {
        return try {
            val questions = remoteDataSource.question.getPinnedQuestions()
            Result.Success(questions.map { it.toDomain() })
        } catch (e: PostgrestRestException) {
            Result.Error(Error.Unknown("Database error"))
        } catch (_: IOException) {
